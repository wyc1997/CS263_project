...
dataset = load_dataset('json', data_files='data/sat-en.jsonl')['train']
...
for i in tqdm.tqdm(range(prompting_range_start,prompting_range_end)):
    context = dataset["passage"][i]
    question = dataset["question"][i]
    options = dataset["options"][i]
    goldenResponse = dataset["label"][i]
    if context is None or question is None or goldenResponse is None:
        continue
    message_raw = "Context: \n{} \nGiven the above context, select the best options from {} that answers the question: '{}' \nThe answer is: ".format(context, options, question)
    # print(message_raw)
    messages = [
    ...
    with open('./results/mistral_inference_SAT_num_{}-{}.jsonl'.format(prompting_range_start, prompting_range_end), 'a') as result_file:
        result_file.write(json.dumps({"context": context, "question": question, "options":options, "label": goldenResponse, "raw_output": raw_output}))
        result_file.write("\n")
